...
def artcode_i(s):
    """retourne la liste de tuples encodant une chaÃ®ne de caractÃ¨res passÃ©e en argument selon un algorithme itÃ©ratif

    Args:
        s (str): la chaÃ®ne de caractÃ¨res Ã  encoder

    Returns:
        list: la liste des tuples (caractÃ¨re, nombre d'occurences)
    ...
    for i in s[1:]:
        if i == letter:
           ...
           res.append({compteur, letter})
           #print(letter, compteur, res)
           letter = i
           compteur = 1
    return res

def artcode_r(s):
    """retourne la liste de tuples encodant une chaÃ®ne de caractÃ¨res passÃ©e en argument selon un algorithme rÃ©cursif
